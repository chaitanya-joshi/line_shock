// v1.9.1 differs from v1.9 in that the average is saved at regular intervals (for eg. every 500 runs) to avoid loss of data under unforeseen circumstances
// Also, total time spent and average time per run is calculated. Also, run is initialized to 0 instead of 1, and the while loop is adjusted accordingly.
...
// Hist here itself.
    ...
    #define RUNS 20000
    #define times 13
    #define saveruns 2000
	
	int totcell,collcount,cellsize,*molcell, crdtp=0, run, runcount=0;//, *ycell; after runcount reaches 10, data will be saved and will be set to zero back again
	int *HEAD,*LIST,*FLIST;
	...
	double *xpos, *hist, *histtemp, *histtempsave, *histstddev, *histstddevtemp; // histtemp will be used to calculate the histogram for one run and one time/
	// it will be used to calculate its square to add to histstddev. histstddevtemp will store the intermediate data
	// histtempsave will be used to save the intermediate histogram
	////////////////////////////

	/*int RUNS = 2;
	int times=5;*/
	double crdt[times+1];
	double timestamps[times] = {10.0,20.0,50.0,100.0,200.0,500.0,1000.0,1500.0,2000.0,2500.0,3000.0,3500.0,4000.0};
	//FILE *fp[2*times];
	...
	void histo(double* data,double* hist, double* histtemp, double* histstddev, int crdtp);
 	...
 	int main(){
 		clock_t begin, end;
		double time_spent, total_time=0.0, average_time_per_run;
 		FILE *f1,*f2;
 		int i, j;
		...
		while(run<RUNS){
			...
			runcount++;
			run++;
			if(runcount==saveruns){
				for (j=0;j<times;j++){
			 		//crdt[j] = timestamps[j];
			 		snprintf(buffer1, sizeof(buffer1), "den-and-stddev-r%.2lftime%davg-over%d.dat",rr,(int)crdt[j],run);
				...
				for(i=0;i<times;i++){
					for(j=0;j<XCELL;j++){
						fprintf(fp[i],"%d\t%f\t%f\n",j,(double)histtempsave[i*XCELL + j],(double)histstddevtemp[i*XCELL + j]);
				...
				runcount=0; // setting runcount to zero again
			}
			end = clock();
		...
		average_time_per_run = (double)total_time/RUNS;
		printf("total_time:%f\n", total_time);
		printf("average_time_per_run:%f\n", average_time_per_run);
 	}
 	...
 	void initarrays(){
		...
		histtempsave=(double *) malloc((times*XCELL+1)* sizeof(double));
		histstddev=(double *) malloc((times*XCELL+1)* sizeof(double));
		histstddevtemp=(double *) malloc((times*XCELL+1)* sizeof(double));
...
*/
    ...
    void histo(double* data, double* hist, double* histtemp, double* histstddev, int crdtp){
		...
		for(i=0;i<NMOL;i++){
			...
			histtemp[temp] = histtemp[temp] + (double)1.0; // Storing the above histogram temporarily
		}
		...
		for(i=0;i<XCELL;i++){
			...
			histstddev[crdtp*XCELL + i] = histstddev[crdtp*XCELL + i] + (double)histtemp[i]*histtemp[i]/RUNS;
